...
function backupDatabase() {
  ...
  // Nom du fichier backup avec la date et l'heure
  const now = new Date();
  const timestamp = `${now.getDate().toString().padStart(2, '0')}-${(now.getMonth() + 1).toString().padStart(2, '0')}-${now.getFullYear()}_-_${now.getHours().toString().padStart(2, '0')}-${now.getMinutes().toString().padStart(2, '0')}`;
  const backupFilePath = path.join(backupDir, `${timestamp}.zip`);

  // Créer un fichier zip pour sauvegarder les données
  ...
  output.on('close', () => {
      console.log(`[Backup] Données sauvegardées : ${timestamp}.zip (${archive.pointer()} octets)`);
  });
  ...
  // Ajouter tous les fichiers du dossier de la base de données sauf `database.json`
...
module.exports = { backupDatabase,saveDb,loadUser,capitalize,hoursToMs,minutesToMs,secondsToMs,msToHours,msToMinutes,msToSeconds,formatMs,calculateHarvestAmount,updateUserInventory,randomSelection,randomAward,fileUrl };